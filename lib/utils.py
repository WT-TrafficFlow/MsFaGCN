...
def generate_data(dataset_dir):
    ...
    for category in ['train', 'val', 'test']:
        ...
        data['x_' + category] = np.transpose(cat_data['x'], (0, 2, 1, 3))
        data['y_' + category] = np.transpose(cat_data['y'][..., 0], (0, 2, 1))
        # x = cat_data['x'][..., :1]
        # y = cat_data['y'][..., :1]
        if mean is None:
            mean = data['x_train'][..., :1].mean()
        if std is None:
            std = data['x_train'][..., :1].std()
        # yield (x - mean) / std, y
        data['x_' + category][..., :1] = (data['x_' + category][..., :1] - mean) / std
    ...
    return data['x_train'], data['y_train'], data['x_val'], data['y_val'], data['x_test'], data['y_test'], mean, std
...
def compute_val_loss_mstgcn(net, val_loader, sw, epoch, limit=None):
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(tqdm(val_loader, desc="valuing")):
...
def predict_and_save_results_mstgcn(net, data_loader, data_target_tensor, global_step, _mean, _std, params_path, type):
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(data_loader):
            ...
            input.append(encoder_inputs[:, :, :12, :1].cpu().numpy())  # (batch, T', 1)
        ...
        np.savez_compressed('data/MAMBAGCN-' + 'YINCHUAN', **{'prediction': prediction, 'truth': data_target_tensor})
